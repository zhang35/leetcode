...
class Solution:
    def gameOfLife(self, board: List[List[int]]) -> None:
        ...
        def neighbors(i, j) -> int:
            cnt = 0

            # Caution here: i > 0 not i > 1
            if i > 0 and board[i-1][j] in (1, -1):
                cnt += 1
            if i > 0 and j > 0 and board[i-1][j-1] in (1, -1):
                cnt += 1
            if i > 0 and j < n - 1 and board[i-1][j+1] in (1, -1):
                cnt += 1

            if j > 0 and board[i][j-1] in (1, -1):
                cnt += 1  
            if j < n - 1 and board[i][j+1] in (1, -1):
                cnt += 1

            if i < m - 1 and board[i+1][j] in (1, -1):
                cnt += 1
            if i < m - 1 and j < n - 1 and board[i+1][j+1] in (1, -1):
                cnt += 1
            if i < m - 1 and j > 0 and board[i+1][j-1] in (1, -1):
                cnt += 1
        ...
        for i in range(m):
            for j in range(n):
                if board[i][j] == 1 and neighbors(i, j) not in (2, 3):
                    board[i][j] = -1
