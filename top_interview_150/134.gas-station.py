...
class Solution:
    def canCompleteCircuit(self, gas: list[int], cost: list[int]) -> int:
        # 1. Only if total gas < total cost, it would return -1
        # 2.1 Suppose we have a station C between A and B. When we arrive at C, the tank must have gas no less than 0; 
        # 2.2 If we choose to start from C, which has zero gas, it will only make it hard to pass station B rather than help it
